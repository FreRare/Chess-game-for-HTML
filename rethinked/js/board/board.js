...
//!!!
//IMPORTANT-> Call the main() before the divs are created else it will not initialize the game
//!!!
//
//!!!!!
...
//For use you need two html canvas with the following ids:
...
class Board {
    constructor() {
        ...
        this.Pawn = class Pawn extends Piece {
            ...
            get_moves(board) {
                ...
                let moves = [];
                    ...
                    if (y === 6) {
                        for (let i = 1; i < 3; i++) {
                            if (Board._check_piece_on_cell(x, y - i, board) == null) {
                        ...
                        if (Board._check_piece_on_cell(x, y - 1, board) == null) {
                            moves.push({ X: x, Y: y - 1 });
                        }
                    }
                    if (Board._check_piece_on_cell(x + 1, y - 1, board) != null &&
                        Board._check_piece_on_cell(x + 1, y - 1, board).black) {
        ...
        this.Rook = class Rook extends Piece {
            constructor(x, y, black) {
            ...
            get_moves(board) {
                ...
                for (let i = 1; i < 8; i++) {
                    ...
                    let piece = Board._check_piece_on_cell(x + i, y, board);
                    if (piece == null) {
                ...
                for (let i = 1; i < 8; i++) {
                    ...
                    let piece = Board._check_piece_on_cell(x, y + i, board);
                    if (piece == null) {
                        moves.push({ X: x, Y: y + i });
                    } else if (piece.black !== black_turn) {
                        moves.push({ X: x, Y: y + i });
        ...
        this.Knight = class Knight extends Piece {
            constructor(x, y, black) {
            ...
            get_moves(board) {
                ...
                for (let cp of where_to_check) {
                    ...
                    if (Board._check_piece_on_cell(cp.X, cp.Y, board) == null) {
                        moves.push(cp);
                    } else if (Board._check_piece_on_cell(cp.X, cp.Y, board).black !== black_turn) {
                        moves.push(cp);
                    }
                }
                return moves;
        ...
        this.Bishop = class Bishop extends Piece {
            constructor(x, y, black) {
            ...
            get_moves(board) {
                ...
                for (let i = 1; i < 8; i++) {
                    ...
                    let piece = Board._check_piece_on_cell(x + i, y - i, board);
                    if (piece == null) {
        ...
        this.Queen = class Queen extends Piece {
            constructor(x, y, black) {
        ...
        this.King = class King extends Piece {
            constructor(x, y, black) {
            ...
            get_moves(board) {
                ...
                for (let cp of where_to_check) {
                    if (0 <= cp.X && cp.X < 8 && 0 <= cp.Y && cp.Y < 8) {
                        let piece = Board._check_piece_on_cell(cp.X, cp.Y, board);
                        if (piece == null) {
                            moves.push(cp);
                        } else if (piece.black !== black_turn) {
                            moves.push(cp);
        ...
        this.piece_positions = [
            ...
            new this.Queen(4, 0, true),
            new this.King(3, 0, true),
            ...
            new this.Queen(4, 7, false),
            new this.King(3, 7, false)
        ];
        ...
        this._game_canvas.addEventListener('click', (event) => {
            ...
            let piece_on_cell = Board._check_piece_on_cell(clicked_cell.x_pos, clicked_cell.y_pos, this.piece_positions);
            ...
            if(selected_piece == null){
                ...
                console.log("Piece selected!", selected_piece.to_string());
            }else{
                let moves = this._select_piece_and_get_moves(selected_piece);
                console.log(selected_piece.to_string(), "\nMoves: ", moves);
    ...
    static _check_piece_on_cell(x, y, board) {
        if(typeof x !== "number" || typeof y !== "number"){
            throw TypeError("[!] Invalid argument! (Board.check_piece_on_cell)" + coordinates_to_string(x, y));
        }
    ...
    _highlight_cell(x, y) {
        ...
        let piece_on_cell = Board._check_piece_on_cell(x, y, this.piece_positions); //Checking which piece is on the cell, and how to highlight it
    ...
    _get_valid_moves(piece){
        ...
        for(let move of all_moves){
            ...
            let p = Board._check_piece_on_cell(move.X, move.Y, test_positions);
    ...
    _kick_down_piece(x, y) {
        ...
        let piece_on_cell = Board._check_piece_on_cell(x, y, this.piece_positions);
