...
function create_cpanel() {
    ...
    start_button.style.cssText = back_button.style.cssText = "width: 133px; height: 42px; padding: 3px; margin: 3px; margin-top: 10px;";
...
class Board {
    constructor() {
        ...
        //Setting turn and selected piece to original
        selected_piece = null;
        black_turn = false;
        //Pieces image
    ...
    //
    _check_piece_on_cell(x, y, both = false, board = null) {
        ...
        if (both) {
            if(board){
                let p = board.WHITE.find(element => element.x_pos === x && element.y_pos === y);
                if (p) {
                    piece = p;
                } else {
                    piece = board.BLACK.find(element => element.x_pos === x && element.y_pos === y);
                }
            }else {
    ...
    //
    //Clones the piece positions for check testing
    //
    _clone_positions(){
        let clone = {WHITE:[], BLACK:[]};
        for(let w of this.piece_positions.WHITE){
            clone.WHITE.push(new Piece(w.name, w.y_pos, w.x_pos, w.black, w.status));
        }
        for(let w of this.piece_positions.BLACK){
            clone.BLACK.push(new Piece(w.name, w.y_pos, w.x_pos, w.black, w.status));
        }
        return clone;
    }
    ...
    _is_check(board = undefined) {
        ...
        //
        //Checks if is check for the team on turn
        //
        if (board) {
    ...
    _get_valid_moves(piece) {
        ...
        let test_table = this._clone_positions();
        const team = (black_turn ? test_table.BLACK : test_table.WHITE);
        let test_piece = team.find(x => x.x_pos === piece.x_pos && x.y_pos === piece.y_pos);
        let valid_moves = [];
        const moves = this._get_moves(test_piece);
        console.log("All moves:");
        console.log(moves);

        for(let m of moves){
            /*let piece_on_cell = this._check_piece_on_cell(m.X, m.Y, true, test_table);
            let checker_piece = this._is_check(test_table);
            if(piece_on_cell && piece_on_cell.x_pos === checker_piece.x_pos && piece_on_cell.y_pos === checker_piece.y_pos){
                console.log("Kick");
                this._kick_down_piece(m.X, m.Y, test_table);
            }*/
            console.log(test_table);
            test_piece.x_pos = m.X;
            test_piece.y_pos = m.Y;
            console.log(test_piece.to_string());
            console.log(test_table);
    ...
    //
    _kick_down_piece(x, y, board = null) {
        ...
        if(board){
            if (piece_on_cell.black) {
                board.BLACK = board.BLACK.filter(x => x.status === "active");
            } else {
                board.WHITE = board.WHITE.filter(x => x.status === "active");
            }
        }else {
            if (piece_on_cell.black) {
